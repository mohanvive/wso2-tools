...
public class RoleBasicAuthHandler implements Handler {

    private static Log log = LogFactory.getLog(RoleBasicAuthHandler.class);
    ...
    public boolean processSecurity(String credentials) {
        ...
        try {
            ...
            isAuthenticated = usm.authenticate(tenantAwareUserName, password);

            if (isAuthenticated) {
                String[] roles = usm.getRoleListOfUser(userName);
                if (allowedRoles != null) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("User " + userName + " does not have required authorization roles to access the api ");
                    }
                    return false;
                }

                //If there are no allowedroles specified in the synapse config then all authenticated users are allowed to access the api
                return true;
            }
